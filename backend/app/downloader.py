        ...
        if content.startswith("["):
            try:
                ...
                lines = [
                    ...
                    "# This file is generated by converting JSON cookies",
                    ""
                ]
                for cookie in cookies_list:
                    ...
                    secure_val = "TRUE" if cookie.get("secure") else "FALSE"
                    flag = "TRUE" if domain and domain.startswith(".") else "FALSE"
                    
                    exp_date = cookie.get("expirationDate")
                    expiration = str(int(exp_date)) if exp_date is not None else "0"
...
def _base_ydl_opts() -> dict:
    ...
    cookies_file = _get_netscape_cookies(settings.COOKIES_FILE)
    if cookies_file and os.path.isfile(cookies_file):
        try:
            # Safely verify the file is readable
            with open(cookies_file, "r") as f:
                f.read(1)
            opts["cookiefile"] = cookies_file
        except Exception as e:
            logger.error(f"Cannot read cookies file at {cookies_file}: {e}")
...
async def fetch_video_info(url: str) -> VideoInfoResponse:
    ...
    if not info:
        raise ValueError("Could not extract video information (empty metadata returned).")
